        ...
        while flingTargetActive do
            if selectedTrollTarget ~= "" then
                ...
                if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    hrp.Velocity = Vector3.new(0, 9999, 0) + Vector3.new(math.random(-500, 500), 0, math.random(-500, 500))
                end
            end
            task.wait(0.2)
        end
...
addButton(tab3, "Kirim Target ke Bawah Map 🕳️", function()
    if selectedTrollTarget ~= "" then
        local target = Players:FindFirstChild(selectedTrollTarget)
        if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
            target.Character.HumanoidRootPart.CFrame = target.Character.HumanoidRootPart.CFrame - Vector3.new(0, 150, 0)
        end
    end
end)
